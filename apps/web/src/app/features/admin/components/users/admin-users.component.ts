...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterLink, FormsModule, HighlightPipe, AdminHeaderComponent],
  template: `
...
export class AdminUsersComponent implements OnInit {
  ...
  private isExtended(u: User): u is MaybeExtendedUser {
    return 'isActive' in u || 'suspendedAt' in u || 'suspensionReason' in u;
  }
  ...
  async bulkSuspend(): Promise<void> {
    ...
    for (const u of targets) await this.authService.toggleUserSuspension(u.id, true, 'bulk');
    await this.loadUsers();
  ...
  async bulkReactivate(): Promise<void> {
    ...
    for (const u of targets) await this.authService.toggleUserSuspension(u.id, false);
    await this.loadUsers();
