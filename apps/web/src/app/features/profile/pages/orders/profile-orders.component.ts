...
@Component({
  ...
  template: `
    <div class="max-w-5xl mx-auto px-4 py-8">
      ...
      @if (!orders().length) {
        <!-- Empty state harmonisé avec Favoris -->
        <div class="bg-white rounded-xl p-8 text-center border">
          <i class="fa-solid fa-receipt text-4xl text-gray-300 mb-3"></i>
          <p class="text-gray-600 mb-3">Aucune commande pour le moment.</p>
          <a class="text-blue-600 hover:underline" routerLink="/">Continuer mes achats</a>
        </div>
      } @else {
        <div class="space-y-4">
