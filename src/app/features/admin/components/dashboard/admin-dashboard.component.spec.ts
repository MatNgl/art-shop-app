...
describe('Page Dashboard Admin (AdminDashboardComponent)', () => {
  ...
  interface SalesDataT {
    date: string;
    revenue: number;
    orders: number;
  }
  interface Privates {
    _salesChartData: { set: (v: SalesDataT[]) => void };
  }

  beforeEach(async () => {
    auth = jasmine.createSpyObj<Pick<AuthService, 'getCurrentUser'>>('AuthService', [
      'getCurrentUser',
    ]);
    productSvc = jasmine.createSpyObj<Pick<ProductService, 'getAllProducts'>>('ProductService', [
      'getAllProducts',
    ]);
