...
describe('Page d’édition de produit (EditProductPage)', () => {
    ...
    function setupWithRouteId(idValue: string): void {
        ...
        categorySvc = jasmine.createSpyObj<Pick<CategoryService, 'getAll'>>('CategoryService', ['getAll']);
        toast = jasmine.createSpyObj<ToastService>('ToastService', ['success', 'error']);

        // valeurs par défaut pour le chargement artistes/catégories
    ...
    it('ngOnInit → ID invalide : affiche une erreur puis revient à la liste', async () => {
        ...
        // artistes & catégories sont chargés avant la vérification d’ID
    ...
    it('ngOnInit → charge artistes/catégories + produit et remplit initial', async () => {
        ...
        expect(component.artists().length).toBe(ARTISTS.length);
