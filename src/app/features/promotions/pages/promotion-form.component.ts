...
import { Promotion, PromotionInput, PromotionScope, PromotionType, DiscountType } from '../models/promotion.model';
  ...
  template: `
    <div class="min-h-screen bg-gradient-to-br from-gray-50 via-orange-50 to-red-50">
        ...
        <div class="max-w-5xl mx-auto px-6 py-8">
          ...
          <div class="flex items-center gap-4">
            <div class="w-16 h-16 bg-white bg-opacity-20 rounded-2xl flex items-center justify-center backdrop-blur-sm">
              <i class="fa-solid {{ isEditMode() ? 'fa-pen-to-square' : 'fa-plus' }} text-3xl text-white"></i>
            </div>
            <div>
              ...
              <p class="text-orange-100">
                {{ isEditMode() ? 'Modifiez les paramètres de votre promotion' : 'Créez une nouvelle promotion pour vos clients' }}
              </p>
            </div>
          </div>
        </div>
      ...
      <div class="max-w-5xl mx-auto px-6 py-8">
        ...
        <div class="pf-progress-sticky bg-white/90 backdrop-blur supports-[backdrop-filter]:bg-white/70 border border-gray-200 shadow-sm rounded-2xl" role="region" aria-label="Préparation de la promotion">
          <div class="px-4 py-3">
            <div class="flex items-center justify-between mb-2">
              ...
              <div class="flex items-center gap-3">
                <span class="text-sm font-semibold" [class.text-green-600]="progress() === 100" [class.text-orange-600]="progress() !== 100">
                  {{ progress() }}%
                </span>
                <button type="button" (click)="toggleProgressDetails()" class="inline-flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-md border border-gray-200 hover:bg-gray-50">
                  <i class="fa-solid" [class.fa-chevron-down]="!progressDetailsOpen()" [class.fa-chevron-up]="progressDetailsOpen()"></i>
                  Détails
                </button>
              </div>
            </div>

            <div class="h-3 bg-gray-200 rounded-full overflow-hidden" role="progressbar" aria-valuemin="0" aria-valuemax="100" [attr.aria-valuenow]="progress()">
              <div class="h-full transition-all" [class.bg-green-500]="progress() === 100" [class.bg-orange-500]="progress() !== 100" [style.width.%]="progress()"></div>
            </div>

            <div class="mt-2 flex items-start justify-between">
              <p class="text-xs" [class.text-green-700]="progress() === 100" [class.text-gray-500]="progress() !== 100">
                @if (progress() === 100) { Prêt à être enregistré 🎉 } @else { Complétez les champs requis pour atteindre 100%. }
              </p>
            ...
            @if (progressDetailsOpen()) {
              <div class="mt-3 border-t pt-3">
                <ul class="grid grid-cols-2 md:grid-cols-3 gap-2">
                  <li class="flex items-center gap-2 rounded-lg px-2.5 py-2 border" [class.border-green-200]="form.get('name')?.valid" [class.bg-green-50]="form.get('name')?.valid" [class.border-gray-200]="!form.get('name')?.valid" [class.bg-gray-50]="!form.get('name')?.valid">
                    <i class="fa-solid text-xs" [class.fa-check]="form.get('name')?.valid" [class.fa-xmark]="!form.get('name')?.valid" [class.text-green-600]="form.get('name')?.valid" [class.text-gray-400]="!form.get('name')?.valid"></i>
                    <span class="text-xs">Nom</span>
                  </li>
                  <li class="flex items-center gap-2 rounded-lg px-2.5 py-2 border" [class.border-green-200]="form.get('type')?.valid" [class.bg-green-50]="form.get('type')?.valid" [class.border-gray-200]="!form.get('type')?.valid" [class.bg-gray-50]="!form.get('type')?.valid">
                    <i class="fa-solid text-xs" [class.fa-check]="form.get('type')?.valid" [class.fa-xmark]="!form.get('type')?.valid" [class.text-green-600]="form.get('type')?.valid" [class.text-gray-400]="!form.get('type')?.valid"></i>
                    <span class="text-xs">Type</span>
                  </li>
                  <li class="flex items-center gap-2 rounded-lg px-2.5 py-2 border" [class.border-green-200]="isCodeValid()" [class.bg-green-50]="isCodeValid()" [class.border-gray-200]="!isCodeValid()" [class.bg-gray-50]="!isCodeValid()">
                    <i class="fa-solid text-xs" [class.fa-check]="isCodeValid()" [class.fa-xmark]="!isCodeValid()" [class.text-green-600]="isCodeValid()" [class.text-gray-400]="!isCodeValid()"></i>
                    <span class="text-xs">Code promo</span>
                  </li>
                  <li class="flex items-center gap-2 rounded-lg px-2.5 py-2 border" [class.border-green-200]="form.get('discountValue')?.valid" [class.bg-green-50]="form.get('discountValue')?.valid" [class.border-gray-200]="!form.get('discountValue')?.valid" [class.bg-gray-50]="!form.get('discountValue')?.valid">
                    <i class="fa-solid text-xs" [class.fa-check]="form.get('discountValue')?.valid" [class.fa-xmark]="!form.get('discountValue')?.valid" [class.text-green-600]="form.get('discountValue')?.valid" [class.text-gray-400]="!form.get('discountValue')?.valid"></i>
                    <span class="text-xs">Réduction</span>
                  </li>
                  <li class="flex items-center gap-2 rounded-lg px-2.5 py-2 border" [class.border-green-200]="isScopeValid()" [class.bg-green-50]="isScopeValid()" [class.border-gray-200]="!isScopeValid()" [class.bg-gray-50]="!isScopeValid()">
                    <i class="fa-solid text-xs" [class.fa-check]="isScopeValid()" [class.fa-xmark]="!isScopeValid()" [class.text-green-600]="isScopeValid()" [class.text-gray-400]="!isScopeValid()"></i>
                    <span class="text-xs">Portée</span>
                  </li>
                  <li class="flex items-center gap-2 rounded-lg px-2.5 py-2 border" [class.border-green-200]="form.get('startDate')?.valid" [class.bg-green-50]="form.get('startDate')?.valid" [class.border-gray-200]="!form.get('startDate')?.valid" [class.bg-gray-50]="!form.get('startDate')?.valid">
                    <i class="fa-solid text-xs" [class.fa-check]="form.get('startDate')?.valid" [class.fa-xmark]="!form.get('startDate')?.valid" [class.text-green-600]="form.get('startDate')?.valid" [class.text-gray-400]="!form.get('startDate')?.valid"></i>
          ...
          <div class="p-6 space-y-5">
            ...
            <div>
              ...
              <div class="grid grid-cols-2 gap-3">
                <label class="relative flex items-center p-4 border-2 border-gray-200 rounded-xl cursor-pointer hover:bg-orange-50 hover:border-orange-300 transition-all">
                  <input
                    type="radio"
          ...
          <div class="space-y-5">
            ...
            <div>
              ...
              <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                <label class="relative flex items-center p-3 border-2 border-gray-200 rounded-xl cursor-pointer hover:bg-blue-50 hover:border-blue-300 transition-all">
                ...
                <label class="relative flex items-center p-3 border-2 border-gray-200 rounded-xl cursor-pointer hover:bg-blue-50 hover:border-blue-300 transition-all">
                  <input type="radio" formControlName="scope" value="subcategory" class="mr-3" />
            ...
            @if (form.get('scope')?.value === 'category') {
              <div>
                ...
                <div class="border-2 border-gray-200 rounded-xl p-4 max-h-64 overflow-y-auto bg-gray-50">
                  <div class="grid grid-cols-2 gap-3">
                    @for (category of categories(); track category.id) {
                      <label class="flex items-center p-3 hover:bg-white rounded-lg cursor-pointer transition-colors border border-gray-200">
                        <input
                          type="checkbox"
            ...
            @if (form.get('scope')?.value === 'subcategory') {
              <div>
                ...
                <div class="border-2 border-gray-200 rounded-xl p-4 max-h-96 overflow-y-auto bg-gray-50">
                  @if (getAllSubCategories().length === 0) {
                    <p class="text-sm text-gray-500 text-center py-4">Aucune sous-catégorie disponible</p>
                  } @else {
                    <div class="space-y-3">
                ...
                <div class="border-2 border-gray-200 rounded-xl p-4 max-h-96 overflow-y-auto bg-gray-50">
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    @for (product of products(); track product.id) {
                      <label class="flex items-center gap-3 p-3 hover:bg-white rounded-lg cursor-pointer transition-colors border border-gray-200">
                        <input
                          type="checkbox"
                        ...
                        @if (product.images && product.images.length > 0) {
                          <img [src]="product.images[0]" [alt]="product.title" class="w-12 h-12 object-cover rounded-lg" />
                        } @else {
                          <div class="w-12 h-12 bg-gray-200 rounded-lg flex items-center justify-center">
                            <i class="fa-solid fa-image text-gray-400"></i>
                          </div>
                        }
                        <span class="text-sm font-medium text-gray-700 flex-1">{{ product.title }}</span>
                      </label>
                    }
                  </div>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl border-2 border-gray-100 overflow-hidden">
          ...
          <div class="p-6">
            <label class="flex items-center gap-4 p-4 border-2 border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50 transition-all">
              <input
                type="checkbox"
              ...
              <div>
                ...
                <div class="text-sm text-gray-500 mt-0.5">La promotion sera immédiatement active si cochée</div>
              </div>
            </label>
          </div>
        </div>
        ...
        <!-- Actions sticky -->
        <div class="sticky bottom-0 bg-white/95 backdrop-blur border-t-2 border-gray-200 px-6 py-4 -mx-4 rounded-b-2xl shadow-2xl">
          <div class="flex justify-between items-center gap-4">
            ...
            <div class="flex gap-3">
              ...
              >
                <i class="fa-solid" [class.fa-save]="!isEditMode()" [class.fa-pen-to-square]="isEditMode()"></i>
...
export class PromotionFormComponent implements OnInit {
  ...
  async onSubmit(): Promise<void> {
    ...
    const input: PromotionInput = {
      ...
      subCategorySlugs: formValue.scope === 'subcategory' ? this.selectedSubCategorySlugs() : undefined,
