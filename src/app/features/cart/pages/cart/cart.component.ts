...
import { ProductPromotionService } from '../../../promotions/services/product-promotion.service';
@Component({
  selector: 'app-cart',
  standalone: true,
  imports: [CommonModule, RouterLink, PricePipe, CartPromotionDisplayComponent, PromotionProgressIndicatorComponent],
      ...
      @if (cart.empty()) {
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div class="bg-white rounded-xl shadow p-4 flex items-center gap-4">
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center justify-between">
                    <div class="min-w-0">
                      <h3
                        class="font-semibold text-gray-900 truncate cursor-pointer hover:text-blue-600 transition"
                        (click)="goToProduct(it.productId)"
                      >
                        {{ it.title }}
                      </h3>
                    ...
                    <div class="text-right">
                      @if (getItemDiscount(it) > 0) {
                        ...
                        <div class="text-xs text-green-600">
                          -{{ getItemDiscount(it) * it.qty | price }} ({{ getItemDiscountPercentage(it) }}%)
                        </div>
                      } @else {
          ...
          <aside class="space-y-4">
            ...
            @if (cartPromotions()?.progressIndicators && cartPromotions()!.progressIndicators.length > 0) {
            <div class="bg-white rounded-xl shadow p-6">
              <app-promotion-progress-indicator [progressIndicators]="cartPromotions()!.progressIndicators" />
            </div>
            }

            <!-- Promotions actives -->
            @if (cartPromotions()?.appliedPromotions && cartPromotions()!.appliedPromotions.length > 0) {
            <div class="bg-white rounded-xl shadow p-6">
...
export class CartComponent implements OnInit {
  ...
  goToProduct(productId: string | number): void {
  ...
  getItemDiscountPercentage(item: any): number {
