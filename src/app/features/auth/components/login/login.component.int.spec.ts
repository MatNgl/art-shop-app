...
describe('LoginComponent — Test d’intégration', () => {
  ...
  it('succès user: redirige vers /catalog et affiche un toast', async () => {
    ...
    authSpy.login.and.returnValue(Promise.resolve(ok) as ReturnType<TAuth['login']>);
  ...
  it('menu mobile: toggle affiche/masque le menu', async () => {
    ...
    const host = fixture.nativeElement as HTMLElement;
    const btn = host.querySelector('[data-testid="mobile-menu-btn"]') as HTMLButtonElement | null;
    const menu = host.querySelector('[data-testid="mobile-menu"]') as HTMLElement | null;
